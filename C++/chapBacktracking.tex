...
static int C[QUEENS];	// C[i]表示第i行皇后所在的列编号
 ...
 * @return 能则返回1，不能则返回0
 */
static int check(const int row, const int column) {
    int ok = 1;
    int i;
    for(i = 0; i < row; ++i) {
 ...
 * @return 无
 */
static void search(const int row) {
...
// 表示已经放置的皇后
// 占据了哪些列
static int columns[QUEENS];
// 占据了哪些主对角线
static int principal_diagonals[2 * QUEENS];
// 占据了哪些副对角线
static int counter_diagonals[2 * QUEENS];
 ...
 * @return 能则返回1，不能则返回0
 */
static int check2(const int row, const int column) {
 ...
 * @return 无
 */
static void search2(const int row) {
    if(row == QUEENS) {  // 递归边界，只要走到了这里，意味着找到了一个可行解
        ++total;
        output();
    } else {
